...
IMAGE_PATH = os.path.join(IMAGE_FOLDER, '{0}_{1:06}_{2}_{3}.png')
...
class CarlaGame(object):
    ...
    def _on_loop(self):
        ...
        if self._control is None or self._timer.elapsed_seconds_since_lap() - self._lasttime >= 0.01:
            ...
            self._control = self._get_keyboard_control(pygame.key.get_pressed(), measurements, self._control)
    ...
    def _get_keyboard_control(self, keys, measurements, prev_control):
        ...
        if keys[K_LEFT] or keys[K_a]:
            if prev_control is not None:
                control.steer = max(prev_control.steer - 0.02, -0.7)
            else:
                control.steer = -0.02
            left_or_right = True
        ...
        if keys[K_RIGHT] or keys[K_d]:
            if prev_control is not None:
                control.steer = min(prev_control.steer + 0.02, 0.7)
            else:
                control.steer = 0.02
            left_or_right = True
    ...
    def _on_render(self):
        ...
        if self._timer.elapsed_seconds_since_lap() >= 0.05:
            if converted_image is not None:
                img = cv2.resize(converted_image, (200, 66), interpolation = cv2.INTER_AREA)
                cv2.imwrite(IMAGE_PATH.format(self._trynumber, self._timer.get_tick(),self._control.steer,self._control.throttle), img)
